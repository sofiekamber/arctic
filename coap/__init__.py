...
def attach_coap(parametric_body: smplx.SMPL, pretrained=True, device=None):
    ...
    if pretrained:
        ...
        if 'mano' in model_type:
            hand_side = 'right' if parametric_body.is_rhand else 'left'
            checkpoint = f'https://github.com/sofiekamber/COAP-checkpoints/blob/main/coap_{model_type}_{hand_side}.ckpt?raw=true'
        else:
